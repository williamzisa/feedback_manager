...
export default function PeoplePage() {
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Errore nel caricamento delle persone"
        );
  ...
  const filteredMentees = mentees
    ...
    .sort((a, b) => {
      const gapA = a.user_sessions?.[0]?.val_gap ?? -Infinity;
      const gapB = b.user_sessions?.[0]?.val_gap ?? -Infinity;
      return gapB - gapA;
    });
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="space-y-1 text-sm text-gray-600">
                        <p>
                          Last Overall:{" "}
                          {hasLastSession && lastSession.val_overall !== null
                            ? lastSession.val_overall.toFixed(1)
                            : "N/A"}
                        </p>
                        <p>
                          Standard:{" "}
                          {hasLastSession && lastSession.level_standard !== null
                            ? lastSession.level_standard.toFixed(1)
                            : "N/A"}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <button
                        onClick={() =>
                          (window.location.href = `/session_results?userId=${
                            mentee.id
