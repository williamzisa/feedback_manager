-- Genera feedback executoin tra tutti gli utenti che condividono uno stesso processo
-- Viene generato un feedback in entrambe le direzioni (`sender A → receiver B` e `sender B → receiver A`).

-- DA SEMPLIFICARE: Include solo i processi che sono associati ai `clusters` della sessione e non ne abbiamo più bisogno

-- Non rimuove duplicati rispetto ad altre regole, ma solo quelli interni a questa: ne abbiamo bisono?

-- Regola 5: Generazione Feedback Execution per coppie di utenti che condividono lo stesso processo 
