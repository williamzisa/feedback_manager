...
- **Navigazione**: Aggiungere link "I miei Processi" in `src/components/navigation/user-menu.tsx` nel menù dell'utente.
...
- **`queries.ts`**: Implementare una nuova funzione (es.✅ `getUserAccessibleProcesses`) che recuperi:
  ...
  - Recuperare dati correlati necessari (es. `questions.description` per i "processes.linked_question_id).
...
### 4. Gestione Assegnazioni Processi

- **`queries.ts`**: Implementare funzioni CRUD per le tabelle `user_processes` e "team_processes"✅
...
- **Logica UI**:
  ...
  - Permettere la creazione di un nuovo processo assegnato al team tramite la pagina di dettaglio del team che si apre da "I miei team" (tabella "team_processes")✅
  - Permettere l'auto-assegnazione di un processo tra quelli appartenenti ai team di cui l'utente fa parte ("user_processes") oppure l'eliminazione di una connessione user_processes che riguardo sé stesso✅

  ***

  - Inserire in src\app\(routes)\teams\[teamId]\page.tsx un componente per la gestione dei team_processes, con la creazione di nuovi processi (tabella processes)
